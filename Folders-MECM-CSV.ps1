...
.NOTES
 Author: Mark Allen
 Created: 27/10/2016
 Modified: Remy Kuster
 added a fourth child
.LINK
        ...
        foreach ($ChildTwo in (($FolderStructure | Where {$_.ChildOne -eq $ChildOne -and $_.ParentRoot -eq $Folder}).ChildTwo) | Get-Unique)
        {
            ...
            foreach ($ChildThree in (($FolderStructure | Where {$_.ChildTwo -eq $ChildTwo -and $_.ChildOne -eq $ChildOne -and $_.ParentRoot -eq $Folder}).ChildThree) | Get-Unique)
            {
                ...
                if(!(New-CMFolder $ChildThreePath)) {continue}
                   ...
                   foreach ($ChildFour in (($FolderStructure | Where {$_.ChildThree -eq $ChildThree -and $_.ChildTwo -eq $ChildTwo -and $_.ChildOne -eq $ChildOne -and $_.ParentRoot -eq $Folder}).ChildFour) | Get-Unique)
                    {
                $ChildFourPath = $ChildThreePath + '\' + $ChildFour
                
                if(!(New-CMFolder $ChildFourPath)) {continue}
            }
        }
    }
}
